...
class AdminUserController extends AbstractController
{
    ...
    public function edit(Request $request, User $user, UserRepository $userRepository, RoleRepository $roleRepository): Response
    {
        ...
        foreach ($user->getRoles() as $key => $role) { 
            $roles[] = $roleRepository->findOneBy(['internalName' => $role]); 
        }
        $user->setRoles($roles);
        $form = $this->createForm(UserType::class, $user);
        $form->handleRequest($request);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $roleIds = $request->request->get('user')['roles']; 
            $roles = [];

            foreach ($roleIds as $key => $i) { //  faux positif à cause du get
                $role = $roleRepository->find($i);
                $roles[] = $role->getInternalName();
            }
